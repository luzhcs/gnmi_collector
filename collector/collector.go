...
func executeSubscribe(ctx context.Context) error {
	if *reqProto != "" {
		...
		//c:= client.New()
		//c.Subscribe(ctx, tq, cfg.ClientTypes...)
...
func displayWalk(target string, c *client.CacheClient, cfg *cli.Config) {
	b := make(pathmap)
...
func produceMessage(result []byte){
